...
export function render(value, options, api) {
  ...
  if (mode === 'editable' && onChange) {
    const input = api.createElement('input', { type: 'datetime-local', style: 'padding: 8px; border: 1px solid var(--color-border); border-radius: var(--border-radius); font-size: 0.875rem;' });
    ...
    span.style.cssText = 'padding: 8px; background: var(--color-bg-body); border-radius: var(--border-radius); font-size: 0.875rem; color: var(--color-text-secondary);';
