// REPL Script: Create default_editor
// A simple JSON editor that works for any item type

const defaultEditorId = crypto.randomUUID();
...
const defaultEditor = {
  ...
  children: [],
  content: {
    for_type: api.IDS.ATOM,  // Works for any type
    is_default: true,
    code: defaultEditorCode
  }
};

await kernel.storage.set(defaultEditor, kernel);
console.log("Created default_editor:", defaultEditorId);
console.log("This editor provides a JSON textarea for editing any item.");
console.log("Test it with: api.editItem('some-item-id')");
