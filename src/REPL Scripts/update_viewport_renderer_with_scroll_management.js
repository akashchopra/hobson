// Update viewport_renderer with scroll state management
// This moves scroll preservation from the kernel to the viewport renderer where it belongs
...
export async function render(item, api) {
  ...
  // Helper: find the correct item element for selection, preferring nested context
  const findItemElement = (target) => {
    // First try to find an element with BOTH data-item-id AND data-parent-id
    // This identifies nested items in containers
    let element = target.closest('[data-parent-id][data-item-id]');

    // If not found, fall back to just data-item-id (for root items)
    if (!element) {
      element = target.closest('[data-item-id]');
    }
...
// Get the viewport_renderer item
const item = await api.get('bd74da77-a459-454a-b001-48685d4b536d');

// Update the code
item.content.code = newCode;
item.modified = Date.now();

// Save it (this updates the modified timestamp)
await api.set(item);

console.log('✓ Updated viewport_renderer with scroll state management');
console.log('✓ Scroll preservation moved from kernel to renderer');
console.log('✓ Cache will be automatically invalidated on next render (modified timestamp changed)');

// Trigger a re-render - this will automatically reload the updated module
// because the kernel detects the cache is stale based on the modified timestamp
await api.navigate(api.viewport.getRoot());
