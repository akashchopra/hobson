// Fix generic_view layout: sticky buttons, scrollable content, expandable markdown
(async function() {
  console.log("Fixing generic_view layout...\n");
...
export async function render(item, viewSpec, api) {
  ...
  for (const [path, hint] of Object.entries(uiHints)) {
    ...
    // Check if this field should expand (markdown fields)
    const shouldExpand = hint.field_view === 'markdown_editable' ||
                         hint.field_view === 'markdown_readonly' ||
                         hint.expand === true;

    if (shouldExpand) {
      fieldElement.style.flex = '1';
      fieldElement.style.minHeight = '200px';
      fieldElement.style.display = 'flex';
      fieldElement.style.flexDirection = 'column';

      // Also make the inner editor/content expand
      const inner = fieldElement.querySelector('.CodeMirror, .markdown-content, pre');
      if (inner) {
        inner.style.flex = '1';
...
`;
  ...
  console.log("Updated generic_view with improved layout:");
  console.log("  - Buttons fixed at bottom");
  console.log("  - Content area scrollable");
  console.log("  - Markdown fields expand to fill space");
