...
**Solution:** Container views now provide their own `siblingContainer` object with an `addSibling()` method. This is passed down via context to children.
...
The kernel no longer knows about layout. Container views inject their layout-aware behavior via the `siblingContainer` object passed through context. Any view can provide its own container implementation for its children.
